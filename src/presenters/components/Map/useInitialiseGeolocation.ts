...
export function useInitialiseGeolocation(setUserLocation: any, setGeoLocStatus: any) {
  useEffect(function initialisePositionTracking() {
    handleGetUserLocation(setUserLocation, setGeoLocStatus);

    // Interval for location updates
    const intervalId = setInterval(() => handleGetUserLocation(setUserLocation, setGeoLocStatus), FREQUENCY);
